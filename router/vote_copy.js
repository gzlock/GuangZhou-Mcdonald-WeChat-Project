...
router.use('/:storeID', (req, res, next)=> {
    ...
    async.series([
                ...
                if (req.query && req.query.code) {//已存在code参数,则获取用户openID
                    return oauth.getAccessToken(req.query.code, (err, data)=> {
                        if (data && data.data && data.data.openid)
                            req.session.openID = data.data.openid;
                        else
                            error = '与微信服务器通讯失败';
                        cb(error);
                    });
        ...
        cb=> {//已获取到openID,判断当天投票数量
            let start = moment().startOf('day').format('YYYY-MM-DD HH:mm:ss'), end = moment().endOf('day').format('YYYY-MM-DD HH:mm:ss');
            req.DB.Vote.count({
                where: {
...
router.post('/:store', (req, res)=> {
    ...
    vote.StoreId = id;
    vote.openID = req.session.openID;
    let now = new Date();
    async.auto({
        vote: (cb)=> {
